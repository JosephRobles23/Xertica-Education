...
1. **Embeddings:** `text-embedding-3-small` (**dimensión nativa 1536**) servido **vía OpenRouter** (endpoint OpenAI-compatible `https://openrouter.ai/api/v1/embeddings`, modelo `openai/text-embedding-3-small`). Se accede vía un puerto nuevo `Embedder` con adapter **real** (`OpenAIEmbedder`, SDK `openai` apuntado al `base_url` de OpenRouter) y **mock** determinista (`MockEmbedder`, vector estable derivado por hash) — así el pipeline corre sin claves (regla de oro). Usa la **`OPENROUTER_KEY`** ya existente (server-side), evitando una credencial extra de OpenAI. Verificado en vivo: embed → vector 1536 normalizado.
