...
| **Source / Fuente** | `models/domain/source.py`, `sources` | Material de referencia que pertenece a una **Ruta** desde Gate 1. Origen `deep_research` (**Vía 1**, con `url` de YouTube) o `upload` (**Vía 2**, con `document_id`). **Solo la Vía 2 alimenta la KB**; las de Vía 1 se **vinculan a un módulo** vía `source_module_links` y NO se ingestan ([[docs/adr/0011-kb-solo-via2-linking-por-modulo]]). Citación a assets M:N vía `asset_sources` ([[docs/adr/0007-source-route-centrica-sourcing]]). |
...
| **Video de Explicación Conceptual** | `Storyboard`, `VideoScene`, `RenderPlan` | Video generado para enseñar un concepto de un Módulo con una secuencia pedagógica clara: problema/pregunta → modelo mental → ejemplo o demostración → checkpoint/malentendido → takeaway. Su duración objetivo es ~90-120 segundos con 5-7 escenas fuertes, cada una con función didáctica explícita; no busca llenar una cuota fija de cambios visuales. Se basa en el contexto del Módulo y en Grounding de la KB; evita secuencias decorativas genéricas como intro abstracta, imagen estática, gráfico aleatorio y captura web sin función didáctica. `ai_video`/Veo es opcional y solo debe usarse cuando la metáfora visual ayude a entender el concepto; no es una intro obligatoria. |
| **Escena Explicativa** | `VideoScene.visual_type` | Escena cuyo visual enseña una relación, paso, contraste, dato o demostración concreta del concepto. Es el default para un Video de Explicación Conceptual y normalmente usa tipos Remotion como `comparison`, `progress_bar`, `callout`, `kpi_grid`, `terminal_scene`, `screenshot_scene` o `text_card`. Los assets generados (`ai_illustration`, `ai_video`) solo se usan cuando aportan un modelo mental o metáfora específica; los gráficos requieren cantidades o comparaciones significativas derivadas del contexto, no datos decorativos. |
| **Visual Didáctico Cualitativo** | `comparison`, `progress_bar`, `callout`, `text_card`, `terminal_scene`, `screenshot_scene` | Visual explicativo que ayuda a aprender sin inventar métricas: muestra contraste, secuencia, decisión, causa-efecto, checklist, flujo o ejemplo trabajado usando lenguaje y relaciones derivadas del Módulo y la KB. Es el fallback preferido cuando no hay datos numéricos; los gráficos cuantitativos (`bar_chart`, `line_chart`, `pie_chart`, `kpi_grid`) solo deben usarse con valores evidenciados o marcados explícitamente como ilustrativos. |
| **Paleta Visual MVP** | `visual_type` | Conjunto preferido de Tipos Visuales para el MVP de Videos de Explicación Conceptual. First-class: `comparison`, `progress_bar`, `callout`, `text_card`, `terminal_scene`, `screenshot_scene`. Condicionales: `ai_illustration` para modelos mentales o arquitectura; `stat_card`, `bar_chart`, `line_chart`, `pie_chart`, `kpi_grid` para valores evidenciados o ilustrativos explícitos; `hero_title` solo para framing/cierre breve; `ai_video` solo para una metáfora significativa, como máximo una vez. |
| **Walkthrough Didáctico** | `screenshot_scene`, `terminal_scene` | Demostración paso a paso que enseña una acción o decisión operativa concreta. En `screenshot_scene` requiere URL específica, propósito, pasos ordenados de UI y resultado de aprendizaje; no basta con abrir una URL genérica y mover el cursor. Si no hay acción verificable que mostrar, el Storyboard debe usar otro Tipo Visual explicativo. |
...
| **Storyboard** | `/ruta/:id/video-storyboard`, `POST /videos/storyboard` | Guion visual del video antes de renderizar. Se produce **KB-grounded** (ADR-0015): la consulta a la KB se arma con `module.titulo`+`module.descripcion` y se envía al puerto `KnowledgeBase.query` route-scoped existente; el humano revisa/edita antes de mandar a `/videos/generate`. El endpoint de storyboard es **puro** (no persiste Asset, no crea Job); `assets` se upsertea en el paso de render, no aquí. |
| **Storyboard Module-grounded** | `StoryboardResponse.grounding` | Storyboard generado desde el Objetivo Pedagógico del Módulo, Ruta y brief cuando la KB no devuelve Grounding útil. Es válido como fallback honesto, pero debe distinguirse de un Storyboard KB-grounded: no finge citas, evita `screenshot_scene` sin URL/acción real y prioriza Visuales Didácticos Cualitativos. |
| **Objetivo Pedagógico del Módulo** | `module.titulo`, `module.descripcion`, `module.tipo` | Intención de aprendizaje que gobierna el Storyboard de un Video de Explicación Conceptual. El Módulo define qué se intenta enseñar; la KB aporta evidencia, ejemplos y vocabulario permitido. `module.titulo` y `module.descripcion` son la fuente principal del objetivo; `module.tipo` actúa como pista de tono y estructura, no como plantilla rígida. Los chunks recuperados no deben reemplazar el objetivo del Módulo ni convertir el video en un resumen aleatorio de resultados RAG. |
| **Intención Pedagógica de Escena** | `pedagogical_intent`, `teaching_point` | Razón didáctica de una escena del Storyboard: qué enseña, por qué ese Tipo Visual es adecuado y qué detalle del Módulo o Grounding la respalda. La narración y el visual deben diseñarse juntos alrededor de esta intención; la variedad visual por sí sola no justifica una escena. |
| **Patrón Didáctico** | `teaching_pattern` | Movimiento pedagógico de alto nivel que guía una o más escenas antes de elegir un Tipo Visual concreto. Ejemplos: corrección de malentendido, explicación de proceso, ejemplo trabajado, modelo mental, regla de decisión, síntesis. El Storyboard debe elegir Tipos Visuales como implementación de un Patrón Didáctico, no como widgets decorativos. |
| **Revisión de Storyboard** | `/ruta/:id/video-storyboard` | Gate humano previo al render que revisa la relación entre Intención Pedagógica de Escena, Tipo Visual y narración. Debe permitir detectar y corregir escenas que no enseñan nada, gráficos sin evidencia, capturas inútiles o visuales mal elegidos antes de gastar Veo/Imagen/TTS. |
| **Render Predecible** | `RenderExecutor`, `RenderPlan` | Ejecución determinista de un Storyboard aprobado. La creatividad y las decisiones pedagógicas ocurren antes del render, durante la generación y Revisión de Storyboard; el render debe materializar lo aprobado con la menor invención posible. |
